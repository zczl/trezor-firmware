...
static void init_wiped_storage(void) {
  ...
  ensure(set_wipe_code(WIPE_CODE_EMPTY, sizeof(WIPE_CODE_EMPTY)),
         "set_wipe_code failed");

  ui_total = DERIVE_SECS;
  ui_rem = ui_total;
  ui_message = PROCESSING_MSG;
  ensure(set_pin(PIN_EMPTY, 0, NULL), "init_pin failed");
}
...
static secbool storage_upgrade(void) {
  ...
  if (norcow_active_version <= 1) {
    if (sectrue != set_wipe_code(WIPE_CODE_EMPTY, sizeof(WIPE_CODE_EMPTY))) {
      return secfalse;
    }
  }
