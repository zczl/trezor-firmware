...
void drawBtnLeft(const char *label) {
    ...
    if (label)
      oledDrawStringCenter(KBD_X_OFFSET / 2 - 1, OLED_HEIGHT - 8, label, FONT_STANDARD);
    else
...
void drawBtnRight(const char *label) {
    ...
    if (label)
      oledDrawStringCenter((OLED_WIDTH + x + 1)/2, OLED_HEIGHT - 8, label, FONT_STANDARD);
    else
...
void drawKeyboard(void) {
  oledBox(KBD_X_OFFSET, KBD_Y_OFFSET, KBD_X_OFFSET + KBD_WIDTH - 1, KBD_Y_OFFSET + KBD_HEIGHT - 1, false);
...
void drawCursor(void) {
  ...
  oledBox(x, FONT_HEIGHT - CURSOR_HEIGHT, x + CURSOR_WIDTH - 1, FONT_HEIGHT - 1, true);
}
